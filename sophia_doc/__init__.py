...
from types import ModuleType, FunctionType, MethodType, MethodDescriptorType
from typing import Any, Dict, List, Optional, Tuple, TypeVar, Union, NamedTuple, Generic
...
class DocNode(Generic[_T]):
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}:{self.name} obj:{self.obj} docstring:{self.docstring}'
        ...
        if self.is_package:
            for importer, modname, ispkg in pkgutil.iter_modules(self.obj.__path__):
                ...
                try:
                    ...
                    warnings.warn(f'Can not import {modname}:\n{traceback.format_exc()}')

        for key, value in inspect.getmembers(self.obj, inspect.ismodule):
            if value.__name__.startswith(self.name + '.') and key not in submodule_names:
        ...
        for name, kind, cls, value in inspect.classify_class_attrs(self.obj):
            if (is_visible_name(name) and cls is self.obj) or \
                ...
                if isinstance(value, cached_property):
                    ...
                    node = DocNode.from_obj(value, name, self.qualname + '.' + name, self.module)
...
class FunctionNode(DocNode[Union[FunctionType, MethodType, MethodDescriptorType]]):
    ...
    def is_async(self) -> bool:
        ...
        return inspect.iscoroutinefunction(self.obj) or inspect.isasyncgenfunction(self.obj)
